...
**Where the safety lives:** `ProjectCeres.Tests/Integration/Authentication/ArchitectureTests.cs::Every_controller_action_declares_authorization_intent` (existing test, already green on `main`). Any new controller action that lacks both `[Authorize]` and `[AllowAnonymous]` fails this test on `dotnet test`.
