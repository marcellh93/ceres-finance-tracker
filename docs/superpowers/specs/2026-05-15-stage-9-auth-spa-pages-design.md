...
Two server endpoints need to ship as part of Stage 9: `POST /api/auth/email/verify` and `POST /api/auth/mfa/disable`.

The first closes a deliberately-deferred slice that originated in Stage 6a (→ see [Stage 6a spec § Why no auto-sign-in?](../../superpowers/specs/2026-05-09-stage-6a-identity-foundation-design.md) lines 463–480). Stage 6a shipped `Program.cs:112`'s `Identity.SignIn.RequireConfirmedEmail = true` as a **deliberate security-first call** — auto-sign-in before email confirmation creates an attack window where an attacker who registers `victim@example.com` (typo or impersonation) gets a working session before the real owner sees a confirmation email. The 6a spec accepted the consequence — *"Real local dev cannot log in until 6c"* — and committed to the email-send + ConfirmEmail handler shipping in Stage 6c. That commitment slipped: 6c.1 shipped password reset only; 6c.2 shipped reauth; email verification slid past 6c entirely and was implicitly redirected to Stage 9 (`roadmap-phase-three.md:954` lists *"`/register` page + email-verification interstitial"* as Stage 9.3, but the roadmap entry no longer carries the 6a rationale anchor — fixed in the same commit as this spec). Stage 9 is therefore the right place to land the email-verify endpoint, the `RegistrationConfirmation` template, and the test-fixture cleanup the 6a spec deferred.

The 6a spec also documented two cleanup options for the test fixture's `SetEmailConfirmedAsync` shortcut (`AuthTestFixture.cs:36–37`), recommending **option B**: keep the shortcut as the fast-path for tests that just need a confirmed user, and add a separate focused test class that exercises the real register → token → confirm → login pipeline end-to-end. Stage 9 takes that recommendation — see Section 6 for the test layout.

The second endpoint is `POST /api/auth/mfa/disable` — the audit-log enum value `MfaDisabled` is reserved but no controller action exists yet. One small server-side prerequisite endpoint also lands first: `GET /api/auth/me`, returning the data the SPA's auth context depends on.
...
- **`POST /api/auth/email/verify`** — accepts a single-use 256-bit Argon2id-hashed token; flips `EmailConfirmed = true`; redirects the SPA to `/login` on success. Plus a small change to `AuthController.Register`: it now issues the token and sends `RegistrationConfirmation` (only on the genuinely-new-account path; the duplicate-email anti-enumeration path stays silent, since sending an email there would itself be an enumeration leak). `EmailConfirmed` stays `false` (the Identity default) until the user clicks the link — closing the 6a-era deferral. Login already refuses unconfirmed accounts (because `RequireConfirmedEmail = true` was set in 6a); Stage 9 wires a new error code `EMAIL_NOT_CONFIRMED` so the SPA can tell the user *why* their otherwise-valid credentials were rejected (see Section 4 for the design rationale on splitting this out from the existing `INVALID_CREDENTIALS` envelope).
...
| **`RegisterConfirmLoginE2ETests`** (new focused E2E class — closes the 6a deferred decision per `2026-05-09-stage-6a-identity-foundation-design.md` lines 476–480) | ~3 | One end-to-end test that exercises the **real** register → token-from-captured-email → `/api/auth/email/verify` → `/api/auth/login` pipeline through the actual endpoints (not the `AuthTestFixture.SetEmailConfirmedAsync` shortcut). Plus a negative test: registering and immediately calling `/api/auth/login` returns `EMAIL_NOT_CONFIRMED` (without going through the verify endpoint). Plus a token-tampering test. The existing `AuthTestFixture.RegisterUserAsync` shortcut stays — most tests still need a confirmed user fast — but this class verifies the production-canonical pipeline once. |
...
| # | Commit | What lands | Doc sync in same commit |
|---|---|---|---|
| 7 | server: `POST /api/auth/email/verify` + register flow change + `RegistrationConfirmation` email + 6a-deferred test-fixture cleanup | New `EmailVerifyTokens` table + EF migration; new endpoint; register issues the verify token + sends the email on the genuinely-new path only (`EmailConfirmed` stays `false`, the Identity default, until the user clicks the link); login already falls through `RequireConfirmedEmail` (set in 6a) and now maps the not-allowed result to a new error code `EMAIL_NOT_CONFIRMED` (split out from `INVALID_CREDENTIALS`); ~12 xUnit tests for the endpoint; **new `RegisterConfirmLoginE2ETests` class** exercises the production-canonical register → confirm → login pipeline end-to-end (closes the 6a deferred decision per [Stage 6a spec lines 476–480](../../superpowers/specs/2026-05-09-stage-6a-identity-foundation-design.md)); `AuthTestFixture.RegisterUserAsync` shortcut stays as the fast-path for tests that just need a confirmed user; `RegistrationConfirmation` enum value + EN + ES resx copy; rate-limits use existing `EmailByUser` + `EmailByIp` policies | `roadmap-phase-three.md` line 1078 ticked; `models.md` updated with new table; `api-contract.md § Canonical error codes` gains `EMAIL_NOT_CONFIRMED` row; `api-contract.md` register-endpoint row updated to reflect the now-actual register → email-confirm → login flow |
