...
2. **Re-home the SPA host by serving the already-built `dist/app.html` via `MapFallbackToFile`, fully dropping Razor.** Deleting `Views/` removes `Views/App/Index.cshtml`, which is the current production host (its `Vite.AspNetCore` `vite-href` TagHelper emits the stylesheet `<link>`, added in the Stage 9 close-out — commit `46f1253`). **Revised 2026-06-29 after `ceres-tech-lead` feasibility read:** the host is NOT net-new. Vite already emits a self-contained `dist/app.html` (the `app.html` rollup input → `src/app/main.tsx`) with the hashed `<script>` AND the stylesheet `<link>` baked in at build time — the Stage 9 "only Razor emits the stylesheet" problem is already solved by Vite's own HTML processing, no TagHelper needed. `run-server.sh` already stages `dist/` → `wwwroot/dist/` in manifest mode (`appsettings.E2E.json` → `"Vite": { "Base": "dist" }`). → Serve the built host via `app.MapFallbackToFile("dist/app.html")`; set Vite `base: '/dist/'` so emitted `/dist/assets/*` URLs match the staged location; port the theme-flash inline script (`ceres.theme:v1`, currently in `index.html`) into `app.html` so the single host carries both it and the existing sidebar-collapsed script; add a `SkipSpaBuild`-guarded MSBuild target mirroring `run-server.sh` step 5 (must NOT fire on inner-loop `dotnet build`/`dotnet test`). Keep `AddViteServices()` for dev. No Razor, no `AddControllersWithViews`.

   **Top risk (tech-lead):** the dev-vs-production divergence that the Stage 9 bug was made of, in reverse. Production serves the static built `dist/app.html`; dev serves a Vite-transformed HTML via `UseViteDevelopmentServer`. It is possible to get production right while dev silently serves the wrong entry (`index.html`/`src/main.tsx` design-system shell instead of `app.html`/`src/app/main.tsx`) or nothing at `/`. **Open question resolved only by a live `dotnet run` check:** does `UseViteDevelopmentServer` serve `app.html` or `index.html` at `/` in dev? The plan makes this an explicit smoke step BEFORE controller deletions land.
