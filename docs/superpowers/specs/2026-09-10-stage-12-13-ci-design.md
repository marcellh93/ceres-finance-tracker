...
- **No real secrets needed.** Build/test/E2E use fixed, obviously-test-only values (the `*_dev_password` role passwords already committed in `scripts/setup-postgres-roles.sql`, and a fixed test-only `Authentication:TokenLookupSecret:Secret` — same posture the test fixture already uses since §12.12). Resend is not needed (Dev/Test → `LogOnlyEmailService`).
