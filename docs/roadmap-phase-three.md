...
- [x] Measured before/after ("Full test suite" step): serial **3m48s** → parallel-clones **3m37s** on CI. The local win is ~2× (2m7s vs 4–5 min on 8+ cores); on the 2-vCPU `ubuntu-latest` runner `MaxParallelThreads=4` is oversubscribed, so CI wall-clock is nearly flat. Correctness/isolation goal met; the CI speedup would need a larger runner.
- [→] **Realise the CI parallel speedup on a larger runner — DEFERRED. Why:** cross-bucket parallelism has almost no cores to exploit on the 2-vCPU `ubuntu-latest` runner (measured ~11s delta); a 4+ vCPU runner would realise the ~2× the design gives locally, but larger runners are a billing/infra decision, not test-code work. **Where:** the receiving `[ ]` lives at **Stage 16 — Hosting & infra** below (§ CI runner sizing).
...
| 16.17 | **Carried in from §12.18 — CI runner sizing.** The integration suite runs parallel-with-clones (`MaxParallelThreads=4`), which gives ~2× locally on 8+ cores but ~0 on the 2-vCPU `ubuntu-latest` runner (measured: serial 3m48s → parallel 3m37s). A 4+ vCPU GitHub runner would realise the parallel speedup on CI. Billing/infra decision, not test-code work. | §12.18 |
