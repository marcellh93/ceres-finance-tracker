...
**Status: ⏳ In progress.** All four SPA surfaces are shipped — `/settings/sessions` (12.1–12.3), `/support` (12.4–12.7, reshaped into the conversation model by 12.6), the email-change surfaces (12.8: the security-page entry point plus `/email-change/confirm` and `/email-change/revoke`), and the reauth dialog wired to every trigger surface (12.9, closed 2026-08-29). Stage 12.5 (IP-anchor / admin ticket list / new-session alert) is separately deferred, user-authorized. The 12.8 Playwright link-click E2E shipped 2026-09-06 (`e2e/auth/email-change.spec.ts`). What remains before Stage 12 closes: the sub-items under §§ 12.8.1–12.8.4 and 12.11–12.12, all of which are recorded as not-scheduled rather than in-flight. Remaining `[ ]` items under this heading are accepted-risk deferrals, future-path items (Stage 13 / 16), or open 12.1-sessions tests.
