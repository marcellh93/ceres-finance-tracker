...
- [ ] 9.5f — CER005 HMAC TokenLookup discipline analyzer: fires on `*Token` classes under `ProjectCeres/Models/` without a `TokenLookup` property + unique-index migration. 0 violations against today's `main`. Tests: new token class without TokenLookup → fires; existing 3 token tables → no fire; non-token model → no fire. `warning` 48h then `error` per C-2. **Add a CER005 row to `AnalyzerReleases.Unshipped.md` (`### New Rules`) in the descriptor commit — RS2008 fails the build otherwise (see ADR-0077 § Release tracking).**
- [ ] 9.5g — CER006 reverse-direction `[PreAuthScope]` analyzer: fires on classes that call `BeginPreAuthUserScopeAsync` without the `[PreAuthScope]` marker. 0 violations after 9.5c's N+1a retro-decoration. Tests: class calls helper without marker → fires; with marker → no fire; doesn't call helper → no fire. `warning` 48h then `error` per C-2. **Add a CER006 row to `AnalyzerReleases.Unshipped.md` (`### New Rules`) in the descriptor commit — RS2008 fails the build otherwise (see ADR-0077 § Release tracking).**
