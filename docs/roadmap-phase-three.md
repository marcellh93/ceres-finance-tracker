...
- [ ] **Batch close-out** — all sub-stages ticked; `dotnet build`, `dotnet test`, `pnpm --dir ProjectCeres.Client build`, `pnpm --dir ProjectCeres.Client test --run` all exit 0; Stop-event hooks ≤10 (Trip-wire C; currently 3); no Trip-wire A/B fired during the batch. Stage flipped to ✅ Done. Per `feedback_finished_stages_have_no_unchecked_items`, no `[ ]` remains under this heading at close-out.
