...
- [ ] **Wire the 12.9 dialog to its trigger surfaces.** The dialog is built and mounted but **no feature surface calls `useStepUp`** — `requireStepUp` has zero non-test callers. `Security.tsx` still catches `ReauthRequiredError` at three reauth-gated call sites (`/api/auth/mfa/enroll` line 44, `TotpEnrollStep1ScanVerify.tsx:33` `/mfa/enroll/verify`, `RegenerateBackupCodesDialog.tsx:37` `/mfa/backup-codes/regenerate`) and renders the static `security.totp.reauthRequired` message instead of opening the dialog, so the user still dead-ends and must re-login manually. Wrap those three call sites in `requireStepUp` and drop the `reauthRequired` state + message branch. The 12.1 sessions list and 12.8 email-change surfaces must call `requireStepUp` when they ship. Found at the 12.9 verification pass, 2026-08-07.
