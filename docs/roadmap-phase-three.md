...
- [ ] **Tripwire (deferred to its own stage):** open a sibling roadmap entry under Batch 5 (likely a sub-stage of Stage 14 or a new Stage 14.x) titled "Enable `EnforceCodeStyleInBuild` + ship a project-wide `.editorconfig` with IDE0001 promoted to a warning". Without that follow-up, IDE0001 remains invisible at build time and regression of this cleanup will only be caught on next manual code reading. Cross-reference both directions: the new tripwire entry cites this stage as its motivation; this checklist item is unticked until the tripwire is opened with a concrete `[ ]` line.
  > **Partial realisation (2026-05-27 via Stage 9.5c):** `Directory.Build.props` (`EnforceCodeStyleInBuild=true`, `AnalysisLevel=latest-recommended`) + a root `.editorconfig` shipped with CER001/CER002/CER004/CER010 analyzers (warning, → error after the 48h soak) + CER020 (error). Stage 9.5c does **not** promote IDE0001 to a warning — that remains the unique tripwire for THIS stage to discharge in Batch 5. The build-infrastructure half (the props + editorconfig files this tripwire would otherwise have to create) now exists; only the IDE0001 severity promotion + the broader code-style normalisation (renames, brace placement, expression preferences — Stage 9.1.6.g territory) remain.
...
| 9.5k | Codified subagent definitions (`.claude/agents/ceres-{architect,tech-lead,pm,cto,security-reviewer}.md`) — read-first contract | Source: 2026-05-26 mid-9.5c-planning discovery. The multi-perspective agent dispatches were framed as reasoning prompts; none was instructed to read the codebase before answering, missing a conflicting architecture test. Codify each role as a named subagent type whose definition (a) names a specific read-list of file paths (not categories — generic instructions decay), (b) requires the response to open with `## What I read` + `## Conflicts found` before the answer. Self-contained per file (no inheritance — platform doesn't support it). Ships before 9.5b and 9.5e so both inherit the discipline. |
...
| 9.5e | 3-agent reviewer pipeline on auth / migrations / IUserOwned diffs (writer + security + playwright-test-audit) | Audit finding: single-agent self-review degrades on reasoning per Huang et al. (arXiv:2310.01798). 3 agents in parallel produce independent reads. Trip-wire A fires if the 3rd reviewer catches what the first 2 missed twice in a row (per L4). Staging-ground only — any rule that fires twice and is syntactically locatable migrates to a Roslyn analyzer or pre-commit hook (per L5). **Prerequisite: 9.5k.** |
...
| 9.5j | Code-fix providers for CER001 / CER004 / CER010 | Analyzers ship in 9.5c with suggested-fix prose only — no IDE quick-fix action. CER004's `DateTime.UtcNow` → `_timeProvider.GetUtcNow().UtcDateTime` substitution is mechanical. CER002 + CER003 + CER020 + CER005 + CER006 have no mechanical fix. Excluded from 9.5c per the brainstorm Section 4 boundary: code-fix providers add ~2x test surface per analyzer. |
...
- [⏳] 9.5c — Roslyn analyzers SHIPPED at `warning` severity (commits `9d6b839`→`ae42c17`→`6f9802c`→`a9b2379`→`892cfe7`→`b4a7239`, 2026-05-27). **Awaiting 48h soak per condition C-2 before warning→error flip** (scheduled ~2026-05-29). CER001/CER002/CER004/CER010 at `warning`; CER020 (resx parity) at `error` from day 1. Retro-decoration: 7 services `[PreAuthScope]`; 15 methods `[RlsBypassJustified("CER-1001".."CER-1015")]` (mapping in spec Appendix A); TimeProvider DI + 21 Pattern A + 4 Pattern B `[AllowsWallClock]` + 23 test fixtures; analyzer csproj + 25 tests pass. **Production CER count: 0.** ADR-0077. Spec: `docs/superpowers/specs/2026-05-26-stage-9-5c-roslyn-analyzers-design.md`. Plan: `docs/superpowers/plans/2026-05-26-stage-9-5c-roslyn-analyzers-impl.md`. **Remaining gate:** Task 8 (severity flip + this checkbox flip to `[x]` + 9.1.6 tripwire cross-reference) lands in one `.editorconfig` commit after the soak.
- [ ] 9.5k — Codified subagent definitions at `.claude/agents/ceres-{architect,tech-lead,pm,cto,security-reviewer}.md` (5 self-contained markdown files with YAML frontmatter). Each: `name`/`description`/`tools` frontmatter; role description ≤3 sentences; named read-list of specific file paths; response-shape contract opening with `## What I read` + `## Conflicts found` before the strategy answer. Smoke-test: dispatch each with a trivial question; inspect the response opens with the preamble. Plus `docs/agents.md` listing all 5 + read-lists + when-to-dispatch, cross-referenced from CLAUDE.md. No skill/hook changes (agents are platform-level).
- [ ] 9.5b — `DualContextWebApplicationFactory` exposes `AppDbContext` (`ceres_app`, RLS-active) + `AdminDbContext` (`ceres_admin`, BYPASSRLS). Startup parity assertion reflects the IUserOwned entity set from the EF model on boot, compares against `pg_class.relrowsecurity` per table; throws on mismatch (fail closed, condition B1). `UserOwnedTables.All` hand-list deleted in the same commit (condition B2). Architecture test pins `Endpoints.*` + `Services.*` to `AppDbContext` unless `[RequiresAdminContext]`. **Prerequisite: 9.5k** — multi-agent dispatch during 9.5b brainstorm/spec/plan MUST use the codified `ceres-*` subagent types.
- [ ] 9.5d — `ProjectCeres.Tests.Integration.AppRole` test project; connection resolves to `ceres_app`. Fixture startup asserts `SELECT rolbypassrls FROM pg_roles WHERE rolname = 'ceres_app'` returns `false` (condition D1). Auth + RLS suites re-run under AppRole in addition to admin-context runs. Stop-hook tier adjusted.
- [ ] 9.5e — 3-agent reviewer pipeline (writer / security / playwright-test-audit). Fires on Edit/Write touching `ProjectCeres/Common/Authentication/**`, `ProjectCeres/Migrations/**`, `ProjectCeres/Models/**` for IUserOwned entities. Condition E1: 90s timeout + `CERES_SKIP_REVIEWER_PIPELINE=1` bypass. Condition E2: third reviewer emits a structured spec-vs-assertion diff. Condition E3: any diff that adds/changes an EF entity requires a migration in the same commit. Trip-wire A armed. **Prerequisite: 9.5k** — reviewer roles dispatch by `subagent_type`.
- [ ] 9.5f — CER005 HMAC TokenLookup discipline analyzer: fires on `*Token` classes under `ProjectCeres/Models/` without a `TokenLookup` property + unique-index migration. 0 violations against today's `main`. Tests: new token class without TokenLookup → fires; existing 3 token tables → no fire; non-token model → no fire. `warning` 48h then `error` per C-2.
- [ ] 9.5g — CER006 reverse-direction `[PreAuthScope]` analyzer: fires on classes that call `BeginPreAuthUserScopeAsync` without the `[PreAuthScope]` marker. 0 violations after 9.5c's N+1a retro-decoration. Tests: class calls helper without marker → fires; with marker → no fire; doesn't call helper → no fire. `warning` 48h then `error` per C-2.
- [ ] 9.5i — typed accessor source generator for `IStringLocalizer<EmailsResource>` consumers: emits `Strings.*` properties matching the resx key set; consumers migrate from string keys. CER020 stays in place as a sibling (additive). Scope: audit consumer call sites, decide typed-accessor vs extension-method shape, ship generator + migrate all consumers in one commit. Tests: every consumer compiles against typed properties; resx renames surface as compile errors.
- [ ] 9.5j — code-fix providers for CER001 / CER004 / CER010: one `CodeFixProvider` per mechanical-fix diagnostic. CER004 `DateTime.UtcNow` → `_timeProvider.GetUtcNow().UtcDateTime` (with injection-point detection); CER001 `BeginTransactionAsync` → `BeginPreAuthUserScopeAsync`; CER010 ticket-format completion. CER002/CER003/CER020/CER005/CER006 out of scope (no mechanical fix). Tests use `VerifyCodeFixAsync`.
- [ ] **Batch close-out** — all sub-stages ticked; `dotnet build`, `dotnet test`, `pnpm --dir ProjectCeres.Client build`, `pnpm --dir ProjectCeres.Client test --run` all exit 0; hook stack still ≤10 (Trip-wire C); no Trip-wire A/B fired during the batch. Stage flipped to ✅ Done. Per `feedback_finished_stages_have_no_unchecked_items`, no `[ ]` remains under this heading at close-out.
