...
**Status: ⏳ In progress.** All four SPA surfaces are shipped — `/settings/sessions` (12.1–12.3), `/support` (12.4–12.7, reshaped into the conversation model by 12.6), the email-change surfaces (12.8: the security-page entry point plus `/email-change/confirm` and `/email-change/revoke`), and the reauth dialog wired to every trigger surface (12.9, closed 2026-08-29). Stage 12.5 (IP-anchor / admin ticket list / new-session alert) is separately deferred, user-authorized. What remains before Stage 12 closes: the Playwright link-click E2E for 12.8, and the sub-items under §§ 12.8.1–12.8.4 and 12.11–12.12, all of which are recorded as not-scheduled rather than in-flight. Remaining `[ ]` items under this heading are accepted-risk deferrals, future-path items (Stage 13 / 16), or open 12.1-sessions tests.
...
- [ ] **`SupportTicket.PrecedingTicketId` is service-enforced only, not structurally.** Postgres's referential-integrity trigger bypasses RLS, so the database will accept a follow-up pointing at another user's ticket; `SupportTicketService.CreateAsync` is the only thing refusing it (pinned at both layers). A composite `(Id, UserId)` FK would make it unrepresentable, as it does for `SupportTicketAttachments`. Deliberately not built with the service half on 2026-08-27: unlike the attachment case, this FK is `Restrict` and read-only, so the residual exposure is an existence oracle over an unguessable GUID rather than the cross-tenant destructive write the attachment shape allowed. `models.md` previously said the FK was "queued with the service half" — corrected. Raised by the Stage 12.5 spec review.
...
- [ ] **Email-change SPA pages (12.8)** — security-page request form (reauth-gated) + `/email-change/confirm` + `/email-change/revoke` token pages (no `/app` prefix; token in the URL fragment — see the sub-stage table above), wired to the live Stage 6.12 API. The emailed links currently dead-end on an unmatched SPA route; tripwire `// FIXME: re-surface in Stage 12` at `EmailChangeService.cs:208`. Add link-click E2E coverage to `ProjectCeres.Client/e2e/` in the same sub-stage (Stage 9.11 deliberately scoped it out — no page existed to drive).
  ...
  - [ ] **Playwright link-click E2E still owed.** The Vitest cases drive the pages directly; nothing yet clicks a real emailed link end-to-end. Stage 9.11 scoped this out because no page existed to drive — that reason is now gone.
