...
- [ ] 9.1.7.a — code-simplification sweep over `ProjectCeres.Tests/` (narrowed out of 9.1.6.g, 2026-06-15). Same discovery-then-fix procedure + findings table + test-count-invariant gate (count before == after; no test added/modified/skipped per `feedback_never_skip_tests_to_make_them_pass`). Note: IDE0001 name-simplification is already done solution-wide (9.1.6.f); this sweep is the OTHER classes only.
- [ ] 9.1.7.b — code-simplification sweep over `ProjectCeres.Client/src/` (narrowed out of 9.1.6.g, 2026-06-15). Routes through `frontend-orchestrator` → `vercel-react-best-practices` (e.g. unnecessary `useMemo`/`useCallback` removal, `?.` chains). `pnpm build` + `pnpm test` exit 0, no bundle-budget regression.
