...
- **File attachments on edit** — the Edit Transaction form lists existing attachments and includes a file input for adding a new one. The new attachment is uploaded as part of the same Save Changes submission — there is no separate Upload button. Each existing attachment has a Remove button that hard-deletes both the DB record and the file on disk atomically. The Remove forms are rendered outside the main `<form>` element (HTML does not allow nested forms) and linked to their submit buttons via the `form=` attribute.
