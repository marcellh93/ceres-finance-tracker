...
- **Paid restoration service** — when a user who churned naturally (no erasure request) returns after their grace period but before permanent deletion, restoring their archived data is a paid service. The fee is set at archive creation time. Users who submitted a GDPR erasure request cannot use this service — they start from zero. See ADR-0029 for the full lifecycle and `CustomerArchive` schema. Do not build this before Phase 5.
...
- [x] Pagination strategy for Transaction History — resolved: see ADR-0027. Phase 1: default view shows the most recent 50 transactions with no pagination controls; an advanced search / date range filter (start date + end date) returns all records in the range with no cap. Phase 3: switch to offset-based pagination (`?page=1&pageSize=50`). Cursor-based pagination deferred unless Phase 3 scale warrants it.
