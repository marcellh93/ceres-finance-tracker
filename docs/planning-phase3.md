...
## Stage 6.15 — Argon2id-O(N) DoS vector on token verify (planned)

**Status:** ❌ Pending. Tracked in `roadmap-phase-three.md` § Stage 6 sub-stages table row 6.15. Sequenced after Stage 6.14 (audit log) and the lockout self-service unlock endpoint per Stage 6c sequencing decision (2026-05-11). Spec: [`docs/superpowers/specs/2026-05-11-stage-6-15-token-lookup-design.md`](superpowers/specs/2026-05-11-stage-6-15-token-lookup-design.md).
...
⚠️ **The current test green-light is misleading.** Tests pass only because the post-run cleanup keeps the token tables empty. Production has no such cleanup — `PasswordResetToken` rows accumulate for 15 minutes, `EmailChangeToken.VerifyNew` rows for 30 minutes, `EmailChangeToken.RevokeOld` rows for **7 days**. The DoS-amplification bug exists in dev/staging/production right now while every relevant test is green. This is exactly the failure mode `feedback_test_edge_cases_as_ship_gate` warns against ("manipulate tests to get a green light and leave the codebase with the security gap"). Stage 6.15's ship-gate explicitly **removes** `AuthTestTokenCleanup` and the 12 `IAsyncLifetime.DisposeAsync` hooks that call it — the new DoS-amplification regression tests (which insert N=200 dummy rows and assert verify completes in <1s) replace the safety the cleanup was providing.
