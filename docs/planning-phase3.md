...
- **Support ticket system** — users submit support requests and hold a two-way **conversation** with the operator (Stage 12.6). A `SupportTicket` owns an ordered `SupportMessage` thread; attachments hang off a message. Email notification to a configurable admin address on new activity; user gets emailed operator replies. Status: `Open`, `Pending`, `OnHold`, `Solved`, `Closed` (Solved is reopenable by a user reply; Closed is terminal, continued via a follow-up ticket). Priority: `Low`, `Normal`, `High`, `Urgent` (advisory).
...
- **Localization (EN / ES)** — the app ships in English and Spanish. See [`docs/superpowers/specs/2026-04-28-localization-design.md`](superpowers/specs/2026-04-28-localization-design.md) for the full design. Key decisions:
  ...
  - **Two translation layers:** React SPA uses `react-i18next` + JSON files (`en.json`, `es.json`). Server-side emails and reports use `.resx` files (`Emails.en.resx`, `Emails.es.resx`, `Reports.en.resx`, `Reports.es.resx`) via `IStringLocalizer`. Server-side email plumbing shipped 2026-05-15 in Stage 8b (9 templates × EN + ES).
...
| 3c.5 | RLS defence in depth | ✅ Done (2026-05-14) | PostgreSQL Row-Level Security live on 24 user-owned tables. `DbConnectionInterceptor.ConnectionOpenedAsync` sets `app.current_user_ref` once per connection (not per command — the spec's per-command `SET LOCAL` pattern was incorrect for this codebase because EF Core 7+ bulk operations (`ExecuteDeleteAsync`/`ExecuteUpdateAsync`) don't open an EF transaction and `SET LOCAL` outside a transaction is a no-op; 10+ production paths would have silently bypassed the GUC). Three-role separation (`ceres_app` / `ceres_admin` with `BYPASSRLS` / `ceres_migrator`); architecture parity test between `UserOwnedTables.All` and `pg_policies`; 15-test RLS integration suite (Groups 1–4 + parity) proves `FromSqlRaw` bypass, `WITH CHECK` insert blocking, admin-role escape, and pooled-connection GUC-reset. `BackgroundJobScope` doorway refusal pinned by architecture test (`.EnterAs(` only allowed in `BackgroundJobScope.cs`). Privilege-leak startup check refuses DDL-capable runtime role. Originally deferred to Phase 4 by ADR-0065; overturned by [ADR-0068](decisions/ADR-0068-postgres-rls-as-phase-3-defence-in-depth.md) (2026-05-09). 1009/1009 tests green at close-out. See [`roadmap-phase-three.md` § Stage 7.5](roadmap-phase-three.md). |
| 3c.6 | Error legibility + OCP cleanup | ⚠️ In progress (started 2026-05-14) | Seven-row infrastructure-only cleanup of diagnostic + OCP gaps surfaced during Stage 7.5: (1) WAF test-mode returns exception bodies ✅ `33e00cd`, (2) `RlsPolicyViolationException` wrapping `SqlState 42501` on user-owned tables ✅, (3) `AffectedRowCount` enforcement helper for bulk operations with exactly-one-row semantics ❌ **resume here**, (4) iterate `UserOwnedTables.All` in `AppDbContext.OnModelCreating` ✅ `fdbf960`, (5) `DbExceptionTranslator` mapping `SqlState` codes (23505/23503/23502) to typed exceptions ❌, (6) split `TestDbFixture` into three single-responsibility fixtures ❌, (7) `UserContext` discriminated union + `[PreAuthCallSite]` attribute + `UserContextRequiredException` replacing the `Guid.Empty` overload per [ADR-0073](decisions/ADR-0073-user-context-as-discriminated-union.md) ❌. No user-facing behaviour change; ~670 LOC across production + tests + docs. Lands before 3d so the cleanups apply while the Stage 7.5 RLS pattern is fresh in the codebase. 1021/1021 tests green at session pause. Spec (with session-state header naming the three implementation lessons): [`docs/superpowers/specs/2026-05-14-stage-7-6-error-legibility-and-ocp-cleanup.md`](superpowers/specs/2026-05-14-stage-7-6-error-legibility-and-ocp-cleanup.md). See [`roadmap-phase-three.md` § Stage 7.6](roadmap-phase-three.md). |
| 3d | Email service + email security | ✅ Shipped 2026-05-15 | Integrate **Resend** (provider chosen 2026-05-12 — see `planning-resolved.md` § Email provider for Phase 3); configure SPF + DKIM + DMARC per [`security-model.md` § Email Security Rules](security-model.md#email-security-rules) (DMARC `p=none` at launch, advance to `p=reject` after aggregate reports confirm clean sending); recipient lock to authenticated user's verified address; per-user rate limit on email-triggering endpoints; Resend send-only API key in secret store with documented rotation; transactional templates EN + ES (registration confirmation, password reset, password-changed, email-change verify-new, email-change revoke-old, TOTP-disabled warning, TOTP re-enrolled, backup codes regenerated, lockout self-service unlock, new-session alert, account-locked notification). Lands before 3e because auth UI flows depend on a working email service. |
| 3e | Auth SPA pages | ✅ Done (closed 2026-06-29) | `/login`, `/login/totp`, `/register`, `/password-reset` (request + reset-with-TOTP), email-verification interstitial, lockout / unlock screens. Centered card layout (no app shell), language toggle on every auth card, Ceres logo/wordmark placement. Error states: wrong password, expired TOTP, locked account. TOTP setup flow: QR code display, manual-entry fallback, backup-codes download. Backup-codes recovery flow when TOTP device lost. Reauthentication prompts on sensitive operations per [`security-model.md` § Login → Reauthentication](security-model.md#login). **Stage 9.3 (shipped 2026-05-22)** — `/register` SPA page + `/email-verify` interstitial + full server-side email-confirmation token pipeline (entity, service, controller, AuthController three-branch anti-enum, Login `EMAIL_NOT_CONFIRMED` branch). **Stage 9.5 (shipped 2026-05-22)** — `/account/unlock` SPA page consuming the Stage 6.10 lockout-unlock backend; `LockoutUnlockService` URL substring updated to match. 9.9 (language-toggle audit), 9.10 (RLS pre-auth-write audit), and 9.11 (Playwright E2E foundations) shipped by the 2026-06-29 close-out. 9.8 (reauth prompts) was relocated to Stage 12.9 and shipped there on 2026-08-07 (`c8ae817`) — the `<ReauthenticationDialog>` + `StepUpProvider` + `useStepUp` auto-replay. Note that the dialog is mounted but not yet wired to any trigger surface: `Security.tsx` still dead-ends `ReauthRequiredError` on a static message. See `roadmap-phase-three.md` § Stage 12.9. |
...
7. **Import sandbox + admin tooling** (Stage 11.5, added 2026-05-13). **⏸️ ON HOLD (2026-06-29, [ADR-0078](decisions/ADR-0078-import-shelved-from-phase-3-beta.md)) — import is shelved from the beta (Stage 11.9), so its sandbox tooling does not execute in Phase 3.** A developer-facing test environment for the CSV/XLSX import pipeline: same code, separate database (`project_ceres_sandbox`), separate `ASPNETCORE_ENVIRONMENT=Sandbox` launch profile, separate port. Cannot contaminate real data — the boundary is physical (different process, different database, different URL), not a runtime switch inside one app. Adds a complementary `ImportBatch` primitive (one row per import, FK from every produced `Transaction` / `Transfer` / `LiabilityPayment`) that powers per-batch bulk wipe and row-level multi-select delete from a new admin SPA route. The same `ImportBatch` primitive is the substrate for the eventual real-user-facing "undo my import" feature (Phase 4+) — no code is sandbox-only. Architecture pattern + Stage 11.5 placement decision: [ADR-0072](decisions/ADR-0072-import-sandbox-as-separate-environment.md). Sits in Batch 4 because the admin SPA route lands cleanly only after the Razor/SPA boundary is gone.

Net result: a clean URL space with one one-shot legacy redirect, a pure Web API + SPA hosting model, and a zero-warning lint baseline. (The developer-only import sandbox is on hold — import is shelved from the beta per [ADR-0078](decisions/ADR-0078-import-shelved-from-phase-3-beta.md); Stage 11.9 removes the import surface, Stage 11.5 resumes only if import is un-shelved.)
...
| Sub-batch | Scope |
|---|---|
| Sessions + Support SPA pages | `/settings/sessions` — active-sessions list with per-row revoke, IP-block toggles, and the reauthentication gate per [`security-model.md` § Reauthentication for sensitive operations](security-model.md#login). `/support` — ticket form + list, `SupportTicket` entity if not already present, email notification to admin on new ticket. Both depend on Batch 3 auth. Also owns the **email-change SPA pages** (settings request form + `/app/email-change/confirm` + `/revoke` token pages, roadmap Stage 12.8) — the Stage 6.12 API emails links to routes with no React page behind them; queued 2026-06-11 by the Stage 9.11 audit. |
| GDPR baseline | Privacy policy text + page; cookie consent banner per AEPD 2024 guidelines per [`security-model.md` § Cookie Consent](security-model.md#cookie-consent-eprivacy--aepd-2024-guidelines); data retention policy enforced (audit log 12-month auto-purge, failed-login 1-year retention purge — both run as [ADR-0067](decisions/ADR-0067-background-job-user-scope-with-iuserscope-and-runner.md) flat cross-tenant background jobs on an external-cron trigger); full ZIP data export as async background job (request → 202 Accepted → email when ready, 24h rate limit, audit-logged); right-to-erasure flow with reauthentication gate; breach notification runbook documented; Records of Processing Activities (RoPA) documented. See [`legal.md`](legal.md). |
...
- Timezone handling — transaction dates stored as local date with no timezone; must decide on a strategy before Phase 3 launch (ADR-0009 follow-up). **Audit 2026-05-12, revised 2026-05-13.** 28 production sites read "today" via `DateTime.Today` / `DateOnly.FromDateTime(DateTime.Today)` server-side — that's the **host machine's** local day (depends on container `TZ`), not the user's local day. Hits: `DashboardService` (5), `DashboardApiController` (6), `CategoryBudgetsApiController` (1), `ReportsApiController` (4), `RecurringTransactionService` (1), report generators (4), and 7 ViewModel defaults.
  ...
  **Options:**
  - **(A) Server-UTC.** Sweep `DateTime.Today` → `DateTime.UtcNow.UtcDateTime`. Predictable failure mode (every user off by N hours from UTC). No schema change. Smallest blast radius. Beta users in non-UTC zones see slightly-stale "today" boundaries near midnight.
  - **(B) Per-user IANA timezone.** Add `TimeZoneId` to Settings, add picker to Stage 15.5 Preferences, inject `IUserTimeZone` service. Correct for all users. Bigger change; needs migration + ADR + Preferences UI.
  ...
  Decision deferred to Stage 15.5 (Preferences/onboarding) so the choice between (B) and (C) can be made alongside the Preferences screen design. No code change yet — the 22 reading sites still drift to server-local, but the user-visible impact for SPA users is contained to dashboard/report boundary cells near midnight. New ADR to be written at decision time supersedes ADR-0009's "before Phase 3 launch" follow-up.
...
5. **`FailedLoginAttempt` retention purge** (Stage 13) — 1-year flat cross-tenant `DELETE WHERE OccurredAt < now() - interval '1 year'`. `AuditLog` uses the **same** flat cross-tenant sweep at a **12-month** cadence (Stage 13 decision 2026-09-19: 12-month SOC 2/ISO/PCI-aligned retention, cross-tenant not per-user — supersedes the earlier "6 months, per-user fan-out"). Both run via the external-cron trigger (`SweepSessions` pattern).
...
## Stage 9.1.5 deferred decisions (2026-05-17)

The following item was surfaced during a Phase 1 login-regression diagnosis, then queued into the Stage 9.1.5 bug batch (sub-stage 9.1.5.h):

1. **`ILookupNormalizer` swap without backfill broke login for pre-existing users** — Commit `4b35911` (Stage 9.1.5.b consolidation work) replaced Identity's default `UpperInvariantLookupNormalizer` with a custom `LowercaseLookupNormalizer` in DI. The swap took effect immediately on the read path: `UserManager.FindByNameAsync` started looking up `AspNetUsers` rows by `WHERE NormalizedUserName = '<lowercase>'`. But existing rows in `AspNetUsers.NormalizedEmail` + `NormalizedUserName` still held uppercase strings from before the swap. Lookups returned zero rows → `SignInManager.PasswordSignInAsync` short-circuited to "invalid login" → `AccessFailedCount` never incremented (the password was never even hashed against the stored hash). Locally only one user existed (the dev seed at commit `cd34735`) and was fixed in-session with `UPDATE "AspNetUsers" SET "NormalizedEmail" = LOWER("NormalizedEmail"), "NormalizedUserName" = LOWER("NormalizedUserName") WHERE "Email" = '<addr>'`. In a production fleet with real users this would have broken every existing login at the moment `4b35911` deployed, with no observable signal in `AccessFailedCount` / lockout state to point at the cause. Fixed in Stage 9.1.5.h: (1) ship `BackfillAspNetUsersNormalizedToLowercase` migration (idempotent — `LOWER(LOWER(x)) = LOWER(x)`), (2) codify the standing rule in `security-model.md` and `models.md` that any `ILookupNormalizer` registration change requires a same-commit EF migration backfilling the two normalized columns. The rule is a special-case of the general "swap function = backfill stored outputs" principle. Tests don't catch this regression by default because every integration test fixture creates its users *after* DI is built, so all test rows are written with the current normalizer's output and never expose the case mismatch.

## Stage 7.5 → Stage 9.10 — RLS pre-auth-write audit (2026-05-18)

Surfaced during Stage 9.6 manual verification on 2026-05-18. The Stage 7.5 `RowLevelSecurityInterceptor` RESETs `app.current_user_ref` on every connection acquisition for any endpoint tagged `[PreAuthCallSite]`. That is correct for unauthenticated reads but blocks every pre-auth path that legitimately needs to INSERT/UPDATE on a known user's behalf (the row's `UserId` mismatches the GUC = NULL → policy fails closed → SQLSTATE `42501` on writes, zero rows on reads of user-owned tables).

The 2026-05-18 fix (`ec63ebd` + the follow-up backup-code fix) introduced `PreAuthUserScope` — a disposable transaction that pins a single connection and `SET LOCAL`-s the GUC — and applied it to four sites: `TotpReplayGuard.TryAcceptLockedAsync`, `MfaBackupCodeService.VerifyAndConsumeLockedAsync`, `PasswordResetService.RequestAsync`, `LockoutUnlockService.IssueAsync`. Those were the sites the manual verification exercised.

**Executor — Stage 9.10** (newly opened 2026-05-18, sub-stage of Stage 9 — Auth SPA pages, per `roadmap-phase-three.md` § Stage 9 sub-stages table line 9.10). Folded INTO Stage 9 rather than deferred so the audit happens inside an active stage with `[ ]` checklist items. The full audit-item shape lives in `roadmap-phase-three.md` § Stage 9 § "Stage 9.10 — RLS pre-auth-write audit + test-infrastructure parity"; the four checklist items there are the source of truth. This section is the originating-context anchor that named the bug.

Audit-scope summary (full bullet list in the roadmap):

1. **Audit every other `[PreAuthCallSite]` endpoint for the same anti-pattern.** Specifically: read or write of any user-owned table from inside `PasswordResetController.Confirm` → `PasswordResetService.ConfirmAsync`, `LockoutUnlockController.Confirm` → `LockoutUnlockService.ConfirmAsync`, `EmailChangeController.Confirm` → `EmailChangeService.ConfirmAsync`, `EmailChangeController.Revoke` → `EmailChangeService.RevokeAsync`, and `AuthController.Register` (`UserManager.CreateAsync` + `_categorySeedService.CopyDefaultsForUserAsync` + `_auditLog.RecordAsync`). For each: wrap with `PreAuthUserScope` where needed and add a regression test.
2. **Decide test-infrastructure parity strategy** for `WafCollection.cs`'s `ApplicationConnection` override to `ceres_admin` (BYPASSRLS) — every existing auth integration test runs with RLS silently inert. Either flip back to `ceres_app` or ship a parallel `AuthRlsParityTestCollection`. Document the decision in `docs/testing.md` § Rules.
3. **Extend `ProjectCeres.Tests/Integration/Rls/PreAuthWritesUnderRlsTests.cs`** with one `[Fact]` per call site the audit finds — the file is already the regression-test home for this class.
4. **Sweep `security-model.md` § Row-Level Security** with a "pre-auth write pattern" section documenting `PreAuthUserScope` as the standing rule.

Tripwire: `ProjectCeres.Tests/Integration/Rls/PreAuthWritesUnderRlsTests.cs` exists and pins the contract for the four sites already fixed. Each item Stage 9.10 ticks lands a new `[Fact]` in that file, so future regressions surface as test failures.
...
- EF global query filters on 22 concrete entities — see roadmap checklist. `Movement` is filtered as the abstract TPC root (EF rejects filters on concrete subtypes of a TPC mapping). Attachments are NOT given an EF query filter (spec §4.1 — they scope via parent in service code), though since Stage 7.5 they carry a denormalized `UserId` column + RLS policy at the database layer. Two `PendingModelChangesWarning`s flag the parent-attachment FK pair — acceptable; documented inline in `ConfigureGlobalQueryFilters`.
