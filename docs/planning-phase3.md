...
## Stage 7.5 deferred items (2026-05-18) — RLS pre-auth-write audit
...
**Receiving `[ ]` item — audit every other `[PreAuthCallSite]` endpoint for the same anti-pattern.** Specifically: read or write of any user-owned table from inside `PasswordResetController.Confirm` (`PasswordResetService.ConfirmAsync`), `LockoutUnlockController.Confirm` (`LockoutUnlockService.ConfirmAsync`), `EmailChangeController.Confirm` (`EmailChangeService.ConfirmAsync`), `EmailChangeController.Revoke` (`EmailChangeService.RevokeAsync`), and `AuthController.Register` (`UserManager.CreateAsync` + `_categorySeedService.CopyDefaultsForUserAsync` + `_auditLog.RecordAsync`). For each: trace the read/write paths, wrap them in `PreAuthUserScope` if they touch a user-owned table, add a regression test under `ProjectCeres.Tests/Integration/Rls/PreAuthWritesUnderRlsTests.cs` (the test runs against `ceres_app` so the bug actually surfaces).

Also: **the integration-test infrastructure (`WafCollection.cs`) overrides `ApplicationConnection` to point at `ceres_admin` (BYPASSRLS)**, meaning every existing auth integration test runs with RLS silently inert. The audit should produce a plan for either (a) flipping the override back to `ceres_app` for auth tests (with the test-data setup necessary to satisfy the policy), or (b) adding a parallel test suite that runs the same auth flows against `ceres_app`. Either approach catches future regressions of this class.

Tripwire: `ProjectCeres.Tests/Integration/Rls/PreAuthWritesUnderRlsTests.cs` exists and pins the contract for the four sites already fixed. The audit's first deliverable is to extend that test file with cases for every other `[PreAuthCallSite]` endpoint that reads/writes user-owned tables.
