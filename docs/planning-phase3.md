...
## Stage 7 — Multi-tenancy cutover (in progress, 2026-05-12)

**Status:** 🚧 Commit 1 of 2 shipped (Tasks 1–9). Spec at [`docs/superpowers/specs/2026-05-12-stage-7-multi-tenancy-cutover-design.md`](superpowers/specs/2026-05-12-stage-7-multi-tenancy-cutover-design.md); plan at [`docs/superpowers/plans/2026-05-12-stage-7-multi-tenancy-cutover.md`](superpowers/plans/2026-05-12-stage-7-multi-tenancy-cutover.md). 938/938 tests green.
...
**Commits.** `7967687` Task 1 · `c0ff081` Task 2 · `a0a149c` Task 3 · `7414c0f` Task 4 · `b29e532` Task 5 (with `06361eb` follow-up) · `b6a266d` Task 6 · `e32b4eb` Task 7 · `8e7d2a5` Task 8 · `372ed95` Task 9. All on `main`. No worktree/branch — per `feedback_stay_on_main`.

**What's still ahead (Commit 2).**

- Task 10: architecture test for the `IgnoreQueryFilters()` allow-list. The test scans `ProjectCeres/**/*.cs` for `IgnoreQueryFilters(` and asserts every hit is in an allow-listed file (`Admin/`, `UserJobRunner.cs`, `CategorySeedService.cs`, the three token services, the future retention-sweep services). Also asserts every expected entity has a filter, `FailedLoginAttempt` does NOT.
- Task 11: IDOR integration suite — cross-tenant 404 assertions on every user-owned API surface, plus the negative-assertion test that proves the global filter alone catches a leak even when the service-layer `.Owned()` chain is bypassed.
- Task 12: empty-DB boot regression — confirms no `IHostedService` or boot-time hook queries user-owned tables.
- Task 13: Commit 1 close-out (full suite + manual browser pass + sync-docs).
- Task 14+: Commit 2 destructive flow — `FakeCurrentUserAccessor`, sentinel-to-real-user remap migration (hard checkpoint requiring DB snapshot + user consent), drop `IOptionallyUserOwned`/`OwnedOrShared` bridge, delete `SingleUserAccessor` + sentinel seeds, swap 78 test sites.

**Risk profile.** The application-layer filter is in place; the architecture-test allow-list ships in Task 10 to close the "can someone accidentally add `IgnoreQueryFilters()`" gap. Until Task 11's IDOR suite lands, the only cross-tenant assertion the test pack carries is `GlobalQueryFilterTests.Account_query_filtered_to_current_user_via_IUserScope` — that's load-bearing but narrow. Commit 1 ships behind Commit 2 in cutover ordering: the destructive remap migration runs AFTER all of Commit 1's tests are green and the operator has taken a DB snapshot.
