...
| 3c.6 | Error legibility + OCP cleanup | Pending | Seven-row infrastructure-only cleanup of diagnostic + OCP gaps surfaced during Stage 7.5: (1) WAF test-mode returns exception bodies, (2) `RlsPolicyViolationException` wrapping `SqlState 42501` on user-owned tables, (3) `AffectedRowCount` enforcement helper for bulk operations with exactly-one-row semantics, (4) iterate `UserOwnedTables.All` in `AppDbContext.OnModelCreating`, (5) `DbExceptionTranslator` mapping `SqlState` codes (23505/23503/23502) to typed exceptions, (6) split `TestDbFixture` into three single-responsibility fixtures, (7) `UserContext` discriminated union + `[PreAuthCallSite]` attribute + `UserContextRequiredException` replacing the `Guid.Empty` overload per [ADR-0073](decisions/ADR-0073-user-context-as-discriminated-union.md). No user-facing behaviour change; ~670 LOC across production + tests + docs. Lands before 3d so the cleanups apply while the Stage 7.5 RLS pattern is fresh in the codebase. Spec: [`docs/superpowers/specs/2026-05-14-stage-7-6-error-legibility-and-ocp-cleanup.md`](superpowers/specs/2026-05-14-stage-7-6-error-legibility-and-ocp-cleanup.md). See [`roadmap-phase-three.md` § Stage 7.6](roadmap-phase-three.md). |
