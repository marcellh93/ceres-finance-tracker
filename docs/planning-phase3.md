...
| 3e | Auth SPA pages | In progress (9.1–9.7 shipped) | `/login`, `/login/totp`, `/register`, `/password-reset` (request + reset-with-TOTP), email-verification interstitial, lockout / unlock screens. Centered card layout (no app shell), language toggle on every auth card, Ceres logo/wordmark placement. Error states: wrong password, expired TOTP, locked account. TOTP setup flow: QR code display, manual-entry fallback, backup-codes download. Backup-codes recovery flow when TOTP device lost. Reauthentication prompts on sensitive operations per [`security-model.md` § Login → Reauthentication](security-model.md#login). **Stage 9.3 (shipped 2026-05-22)** — `/register` SPA page + `/email-verify` interstitial + full server-side email-confirmation token pipeline (entity, service, controller, AuthController three-branch anti-enum, Login `EMAIL_NOT_CONFIRMED` branch). **Stage 9.5 (shipped 2026-05-22)** — `/account/unlock` SPA page consuming the Stage 6.10 lockout-unlock backend; `LockoutUnlockService` URL substring updated to match. Remaining: 9.8 reauth prompts, 9.9 language toggle audit, 9.10 RLS pre-auth-write audit, 9.11 Playwright E2E. |
