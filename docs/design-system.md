...
- ✅ **Showcase contrast ratios on OKLCH tokens** (resolved 2026-05-09 via T5.20). `parseRgb()` in `src/design-system/lib/contrast.ts` now dispatches on `rgb()`/`rgba()` vs `oklch()` and converts OKLCH→sRGB inline via the standard OKLab matrix (Björn Ottosson's reference, same constants used in T3.15's theme-color computation). The Colors and Charts swatches now show their "vs --foreground: X.XX : 1 (AA)" line for every token regardless of source color space. The try/catch fallback in `SwatchGrid.tsx` was removed in the same commit.
