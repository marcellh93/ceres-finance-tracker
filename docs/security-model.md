...
- **The support operator reply (Stage 12.6) is the first cross-user WRITE under `Admin/`.** `POST /api/admin/support/tickets/{id}/messages` writes an `Agent` message onto another user's ticket. Its safety rests on three layers, verified by an adversarial review: **(1)** the ticket is read via `AdminDbContext` (BYPASSRLS) + `IgnoreQueryFilters()`, and the written message's `UserId` is stamped **explicitly to `ticket.UserId`** taken from that loaded row — never the admin, never a request field (left unset, the `UserOwnershipInterceptor` would stamp the admin and the message would land in the admin's tenant, invisible to the owner); **(2)** the `SupportMessage → SupportTicket` FK is composite `(SupportTicketId, UserId)`, so even under BYPASSRLS a message whose owner differs from the ticket's is **unrepresentable** at the database, not merely unenforced — the RI-trigger backstop RLS itself cannot provide; **(3)** an unknown ticket answers 404 (no oracle) and an illegal status transition is refused by `SupportTicketStateMachine` (422) before any write. The interim owner-stamped `AuditLog` records the action (`SupportMessageByAgent`); the full append-only `AdminAuditLog` remains deferred (roadmap § 15.6).
- **The support ticket list/triage read (Stage 12.5.2) is the first cross-user READ under `Admin/`.** `GET /api/admin/support/tickets` (offset-paginated) and `GET /api/admin/support/tickets/{id}` read **every** user's tickets via `AdminDbContext` (BYPASSRLS) + `IgnoreQueryFilters()` — the intended operator read, not an IDOR: neither endpoint takes a `userId` request param, so the read cannot be steered to a chosen user, and the single-id thread fetch answers **404-not-403** on a miss (no existence oracle). The owner's email is projected from `AspNetUsers` (**that column only** — no other Identity field). This stays inside the ADR-0065 "metadata-only, no transaction-level viewing" line: it exposes ticket subjects/bodies (user-authored support prose) and the owner's email, not financial/ledger data. The list is bounded (`pageSize` clamped 1..100; `MessageCount`/`LastMessageAt` are SQL rollups, bodies never materialised in the list). Gated by the same class-level `[RequireAdmin]` + `[RequiresAdminContext]`; the SPA gate holds **no cached admin flag** — the live `[RequireAdmin]` server check is the sole authority (a hidden route is not an access boundary; a stale client flag is exactly what ADR-0080 rejects). Verified by the 3-agent reviewer pipeline (`47ddba37`/`7ebe4479`).
...
### Session IP Anchoring

**Status: ✅ Shipped (Stage 12.5.1).** Opt-in, per-session defence against a stolen session cookie replayed from a different network.

- **What it defends:** a stolen `__Host-Session` cookie (XSS, a leaked backup, a shoulder-surfed token) replayed from an attacker's machine. The attacker holds the cookie but connects from a different IP, so an anchored session rejects the replay.
- **What it does NOT defend:** an attacker who signs in with the password (and MFA, if enabled) from a new device — that is a fresh, legitimately-created session, not a cookie replay. That case is covered by MFA + the new-device email-link step-up + the new-session-from-new-IP alert (Stage 12.5.3). The `/settings/sessions` toggle copy states this explicitly so anchoring is never mistaken for device-blocking.
- **Enforcement (two points):** (1) `SessionRevocationValidator.OnValidatePrincipal` — after loading the session row, if `session.IsIpAnchored` and `Connection.RemoteIpAddress?.ToString() ?? ""` is not an **exact** match for `session.IpCreatedAt`, it calls `RejectAsync` (RejectPrincipal + SignOutAsync); rides the per-request revocation SELECT, no extra query. (2) `PersistentCookieRotationMiddleware` — the `__Host-Persist` rotation hop runs BEFORE `UseAuthentication`, so it never reaches the validator's check; it therefore enforces the anchor itself (a mismatched-IP rotation is refused, leaving the request unauthenticated — exactly as a bad token does) and carries `IsIpAnchored` forward onto the rotated session row. Without both points, a stolen persistent cookie replayed from another IP could rotate into a fresh valid session and the anchor would silently evaporate on the first return visit.
- **Granularity — exact IP (Fork 1a):** exact-match was chosen over /24 subnet or geo-ASN. Subnet still lets a thief on the same network through and is an arbitrary line; ASN needs an IP→ASN dataset. Because anchoring is opt-in per session, the roaming-lockout cost of exact-match is self-selected away — a user anchors only a session on a network they know is stable.
...
> **Status: ✅ Shipped (Stage 6.12, 2026-05-10).** Endpoints `POST /api/auth/email-change/request|confirm|revoke`, plus `GET /api/auth/email-change/pending` (Stage 12.8). See `docs/superpowers/specs/2026-05-10-stage-6-12-email-change-design.md`; ship-gate covered by 37 integration tests under `ProjectCeres.Tests/Integration/Authentication/EmailChange*`.
...
- **Pending-change disclosure is masked (Stage 12.8).** `GET /api/auth/email-change/pending` backs the settings banner that tells a returning user a change is still in flight — without it the page shows the old address unchanged, the user cannot tell whether their request worked, and they resubmit, triggering two more emails. The target address is masked **server-side** by `EmailMask.Mask` and the full value never leaves the server: masking in the browser would leave the real address in the response body, visible in devtools and to anything reading the traffic, which would look private without being private. The mask keeps the first character of the local part and of the domain, then a **fixed** five-dot run — fixed so that a 1-character local part and a 22-character one render identically, since a variable-width mask leaks length. The extension is dropped entirely: `.co.uk` would narrow the domain to a country, and `localhost`'s *absence* of an extension would announce an internal address, so rendering no extension removes both signals (and makes the multi-part-extension split bug unreachable). Malformed input returns empty rather than echoing the raw value. The endpoint is `[Authorize]` but deliberately **not** `[RequireRecentAuth]` — the settings page reads it on load, so a reauth gate would prompt on every visit; it is safe unbounded precisely because the payload is masked. It reads through the RLS-filtered `AppDbContext` (the caller is authenticated, so no pre-auth bypass applies) with an explicit `UserId` predicate, so a foreign row is filtered at the application layer and, under `ceres_app`, refused by the `user_isolation` policy as well. Both halves are exercised: `EmailChangePendingTests` pins the endpoint output (but runs as `ceres_admin`/BYPASSRLS, so it observes only the application predicate), and `EmailChangePendingUnderRlsTests` (AppRole suite, `ceres_app`) pins the database half with the positive+negative visibility control. The second test exists because the first cannot distinguish "RLS refused it" from "the `.Where` excluded it" — a distinction this sentence previously asserted without evidence. **Threat bounded:** masking defends the shoulder-glance and the screenshot, not an attacker who already holds the session — against that, the revoke link to the old address remains the real control.
...
- **In-app cancel (Stage 12.8.1, 2026-09-07):** `POST /api/auth/email-change/cancel` (`[Authorize]`, not `[RequireRecentAuth]`) lets a signed-in user cancel their own pending change without opening the emailed revoke link — it consumes both sibling tokens for the authenticated `userId`, notifies the old address, and audits `EmailChangeRevoked`. Deliberately not reauth-gated: cancel returns the account to its unchanged status quo, strictly less sensitive than the reauth-gated `/request`; the state-changing direction (`/confirm`) stays token+window protected. Only removes an unconsumed intent — no address mutation, no session revoke, no `SecurityStamp` roll (pinned by `EmailChangeCancelTests`).

#### Accepted risk: no in-app recovery from an already-completed hostile change (Stage 12.8.1 E2, accepted 2026-09-07)

**The gap.** An attacker holding a live session can initiate an email change and click the emailed confirm link within the 30-minute window. Once `/confirm` runs, the address of record is the attacker's, and the legitimate user's 7-day revoke link is already consumed — clicking it returns `InvalidToken`, indistinguishable from "expired." The attacker can then drive password-reset to their new address and re-enter the account. There is **no in-app path** for the legitimate user to reclaim the address; recovery is out-of-band (support) only.

**Why this is an accepted risk for the hosted beta, not a silent hole.** The damage is bounded by controls that already ship: `/confirm` revokes every session and regenerates `SecurityStamp`, so the attacker is logged out the instant they confirm (they cannot *stay* in on the stolen session); and the old address receives an "email changed to X" notice on completion, so the legitimate user — who still controls the old inbox — is told immediately. What is missing is only the *in-app undo* after completion; support can still reverse it. For a beta, "recovery requires a support request" is a tolerable stopgap **provided it is documented with a runbook (below) and the real fix is scheduled** (Stage 15.8.x, § below).

**Support runbook — reversing a hostile completed email change.** When a user reports "my email was changed and I didn't do it":
1. **Verify identity out-of-band** — the requester must prove control of the *old* address (the one that received the change-completed notice) by another factor (e.g. the original registration details, recent activity), since the account's current address is now the attacker's. Do not act on a request that only proves control of the *new* address.
2. **Lock the account first** — force a password reset / disable sign-in for the account so the attacker cannot act during recovery.
3. **Restore the address of record** — until an admin rollback UI exists (Stage 15.8.x / option A), this is a direct, audited data operation by an operator: set `AspNetUsers.Email`/`NormalizedEmail`/`UserName` back to the verified old address and regenerate `SecurityStamp` (kills any session the attacker still holds). Record the operator, timestamp, and reason.
4. **Notify both addresses** of the reversal.
5. **Escalate to the real fix** — every occurrence is evidence for prioritising the grace-period reclaim (option B).

**The real fix is scheduled, not dropped:** a grace-period reclaim (option B) — the change-completed notice to the old address carries a time-boxed "undo this" link that reverses the swap in-app — is filed as a Stage 15.8.x roadmap item. Option A (admin-assisted rollback UI) folds into the same admin surface once Stage 15.8 builds it.
...
**Stage 12.6 — `SupportNotificationService`.** The conversation model added two user-facing emails (an operator reply lands in the user's inbox, and a Solved notice) plus the operator-notify on a new user reply. `SupportNotificationService` is **not** on the allow-list above **by design**: it resolves recipients through `IEmailRecipientResolver` / `ISupportRecipientResolver` (the two factories already listed), so it holds no `EmailRecipient` construction site of its own. The agent-reply email **includes the operator's reply text** (a user decision over notification-only), which makes HTML sanitisation mandatory — the body flows through the composer's `HtmlEncoder`-escaped arg path, pinned by a two-sided negative test that a raw `<script>` renders escaped. The support-thread link in the email uses `Email:PublicBaseUrl` (configured), never a request field, so an operator cannot point a user's link at an arbitrary host; it falls back to the request origin only in dev/test (set `PublicBaseUrl` in Production — Stage 16).
...
**Implementation status (Stage 12.14, 2026-09-11 — [ADR-0081](decisions/ADR-0081-docker-containerization.md)).** The repo `Dockerfile` realises the wave-1 controls a Dockerfile alone can express: non-root uid 1000 on `mcr.microsoft.com/dotnet/aspnet:10.0-alpine`, a multi-stage build whose runtime image carries no SDK / Node / pnpm / package caches, and no baked secrets (all config runtime-injected). It also installs `icu-libs` — the alpine runtime is globalization-invariant by default, which would crash the app's `en`/`es` culture setup. Still pending, tracked at roadmap 16.13 as orchestration/CD-layer flags rather than image content: base-image **digest pinning**, `--cap-drop=ALL` + `--security-opt=no-new-privileges`, explicit `--memory`/`--cpus` limits, the Trivy/Grype CVE scan (CD, 16.11), and the entire wave-2 read-only-filesystem work (the writable-path inventory — `/tmp`, the FileAttachments upload root, the DataProtection keyring — lives in the §12.14 design doc).
