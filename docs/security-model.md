...
- **Re-generation:** when new backup codes are generated (e.g. user requests fresh codes after using one), all existing codes for that user are immediately invalidated before new ones are issued. **Re-generation requires the user's current TOTP code in the request body (`{ totpCode }`)** as a step-up proof of TOTP-device possession — prevents an attacker with a stolen session from invalidating the victim's backup codes. Returns 422 if `totpCode` is missing, 401 if the code is invalid, 409 if MFA is not enabled. (Stage 6b.3; full step-up middleware via `LastPasswordVerifiedAt` deferred to Stage 6c.)
...
- **MFA during reset (conditional on user's MFA-enabled state per ADR-0069):**
  - **If the user has MFA enabled:** the reset flow must require a valid TOTP code (or backup code) before the new password is accepted. This prevents an attacker who has only the reset email (but not the TOTP device) from completing the takeover. If the user has lost their TOTP device, backup codes are the recovery path — not a TOTP bypass in the reset flow.
  - **If the user does not have MFA enabled:** the reset flow accepts the email-link token alone. The email-channel ownership proof (the user clicked the link sent to their address) IS the second factor. Compensating controls remain in place: the email-link is single-use, 15-minute expiry, all sessions revoked on success, separate notification email sent to the address-of-record on completion.
