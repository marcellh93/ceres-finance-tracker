...
| Support operator | POST `/api/admin/support/tickets/{id}/messages` | **Stage 12.6.** `[RequireAdmin]` (live per-request DB role check). One call posts an operator reply and/or sets a chosen status; body `{ body?, status }` — an empty body is a status-only change (no message row). The Agent message is owner-stamped with the **ticket owner's** id, read from the loaded ticket via `AdminDbContext` + `IgnoreQueryFilters()`. 404 on an unknown ticket, 422 on an illegal transition (e.g. status on a Closed ticket), 403 for a non-admin. Rate-limited like the user endpoints (it emails the user). See `security-model.md`. |
