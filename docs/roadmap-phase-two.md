...
4. [Stage 3 — CSV Import](#stage-3--csv-import)
5. [Stage 4 — Transfer Attachments](#stage-4--transfer-attachments)
6. [Stage 5 — Liability Enhancements](#stage-5--liability-enhancements)
7. [Stage 6 — Recurring Reminder Enhancements](#stage-6--recurring-reminder-enhancements)
8. [Stage 7 — Reports](#stage-7--reports)
9. [Stage 8 — Visual Dashboard + React Components](#stage-8--visual-dashboard--react-components)
10. [Stage 9 — Financial Health Metrics](#stage-9--financial-health-metrics)
11. [Stage 10 — CSV Export](#stage-10--csv-export)
...
- [ ] Create a `CategoryBudget` on an Income category → rejected with "Budgets can only be applied to Expense categories"
- [ ] Create two active `CategoryBudget` entries for the same Category + Currency → second creation rejected
- [ ] Create a valid `CategoryBudget` → dashboard progress bar appears; `spent` value updates after recording a transaction in that category
- [ ] Create a `Spending` goal budget → tag a transaction to it → progress bar reflects the transaction amount
- [ ] Create a `Savings` goal budget linked to an account → progress bar reflects the account balance
- [ ] `LinkedAccountId` required when `GoalType = Savings` → null value rejected with validation error
- [ ] Tag a transaction to a goal budget → transaction appears in the goal budget's linked transaction list

### Reconciliation / IsCleared

- [ ] Record a transaction manually → `IsCleared = false` by default; toggle to `true` via Edit view → badge updates
- [ ] Fingerprint matching: import a CSV row with date ±1 day and same amount as an existing transaction → row imported with `IsCleared = false` and "Needs review" badge
- [ ] "Different transaction" reconciliation: user marks the flagged row as a new distinct transaction → row cleared automatically; no manual follow-up required
- [ ] Bulk "Mark all cleared" within a date range → all transactions in range set to `IsCleared = true`

### CSV Import

- [ ] Upload an `.xlsx` file → rejected with "Only CSV files are supported. Please export your bank statement as CSV."
- [ ] Upload `valid_import.csv` with a correctly configured profile → 10 transactions created, all `IsCleared = true`; summary shows "10 imported, 0 flagged, 0 errors"
- [ ] Upload `duplicate_candidates.csv` → flagged rows appear in Transactions Index with `IsCleared = false` and "Needs review" badge; summary shows correct flagged count
- [ ] Create a `CsvImportProfile` → mappings saved; auto-applied on next import of same-format CSV
- [ ] Soft-delete a `CsvImportProfile` → excluded from active list; appears in deleted list with 90-day countdown
- [ ] Recover a soft-deleted profile within 90 days → profile restored to active list
- [ ] Negative debit in CSV → imported as positive amount with correct Expense category direction
...
- [ ] Create Amortising liability account with `InterestRate = null` → rejected with validation error
- [ ] Create FullMonthly liability account with `InterestRate` set → rejected with validation error
- [ ] Create Amortising account with valid interest rate → payoff projection panel appears on account detail page; shows payoff date and total interest
- [ ] FullMonthly account detail page → no projection panel rendered
- [ ] "What if €X extra/month" input → projection updates to show earlier payoff date and lower total interest

### Recurring Reminders

- [ ] `SnapToCalendarDay` reminder confirmed late (e.g. on the 20th, `DayOfPeriod = 15`) → `NextDueDate` advances to the 15th of the following month (skips forward, not backward)
- [ ] `RelativeToLastConfirmation` reminder confirmed on the 20th (monthly) → `NextDueDate` = 20th of next month
- [ ] `ManualDate` reminder confirmed → user prompted for next due date; `DayOfPeriod` field hidden on form
- [ ] `ManualDate` reminder confirmed without setting a new date → validation error "Please set the next due date"
- [ ] Upcoming Payments view: shows all recurring transactions due within 30 days; excludes those due > 30 days away
- [ ] Navbar badge shows correct count of upcoming payments due within 30 days
- [ ] "Due today" rows in Upcoming Payments view are highlighted with amber badge
