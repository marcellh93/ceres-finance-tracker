...
Tracks active authenticated sessions server-side. Enables multi-device support, user-visible session management, per-session revocation, IP enforcement, and the `__Host-Persist` "remember me" rotation flow. One row per active session per device — **enforced** since Stage 12.10 (login revokes the same-device duplicate; see § Session lifecycle below). Inserted on successful login (or successful TOTP step in the MFA flow); marked `RevokedAt = now` on logout, IP block, persistent-token rotation, or same-device supersession.
...
| LastUsedAt | timestamp | NOT NULL | Updated on authenticated requests via `SessionRevocationValidator.OnValidatePrincipal`, debounced to at most one write per 60 seconds (Stage 6b.3). The SELECT still runs on every request (revocation guarantee). Since Stage 12.10 it is also the activity signal the active-sessions display filter and the retention sweep key off (see § Session lifecycle). Future-work options tracked in `planning-future.md` § *Session-validation per-request DB write*. |
...
| IsIpAnchored | bool | NOT NULL, default false | Stage 12.5.1. Opt-in per-session IP anchor. When true, `SessionRevocationValidator` rejects (signs out) any request whose source IP is not an exact match for `IpCreatedAt`. Defends a stolen session cookie replayed from another network; recovery on a legitimate IP change is a fresh login. Toggled from `/settings/sessions` via `POST /api/sessions/{id}/anchor` (reauth-gated). Default false — existing and new sessions are unanchored until the user opts in. |

**Indexes:** `(UserId, RevokedAt)` for the revocation lookup; `(LastUsedAt)` for the retention purge (the `SweepSessions` DELETE, shipped Stage 12.10 — see § Session lifecycle).

**IP enforcement is per session:** enforcement is opt-in per session via the `IsIpAnchored` flag (the toggle shipped in Stage 12.5.1 on `/settings/sessions`). Each anchored session's request is validated against the `IpCreatedAt` of its own session row — not against a single account-wide IP, and using an exact match (Fork 1a; subnet/ASN granularity was considered and rejected as it either weakens the replay defence or needs an IP→ASN dataset). Multiple devices with different IPs are fully compatible, because each device anchors to its own creation IP, and a device that roams simply does not anchor. It defends a stolen session cookie replayed from another network; it does NOT block someone who signs in with the password from a new device — that is covered by MFA and the new-session alert (Stage 12.5.3).
...
**Session lifecycle (Stage 12.10).** A row lives past its cookie's death — there was no expiry concept before 12.10, so the active-sessions list showed sessions whose cookie had long expired, with live Revoke / Block-IP buttons. Three parts fix this, all keyed off the lifetime constants now centralized in `SessionConstants` (`EphemeralSlidingWindow` = 30 min, `PersistentLifetime` = 30 days, `RetentionHorizon` = 90 days):

- **Display expiry filter** — `SessionService.GetActiveAsync` shows a row only if `RevokedAt IS NULL` AND it is still live by `LastUsedAt`: within 30 min for an ephemeral session (the sliding-cookie window), within 30 days for a persistent one. `LastUsedAt` is the activity signal (debounced 60 s), so the boundary is fuzzy by up to ~1 min — acceptable, and it only ever errs toward showing a very-recently-active session. Pure read filter; no data is mutated.
- **Login dedup** — `AuthController.IssueSessionAndCookiesAsync` revokes any live ephemeral row for the same `(UserId, UserAgent, IpCreatedAt)` before inserting the new one (guarded `Id <> newSessionId` so the row being created is never self-revoked; the new cookie's `sid` points at the new row, so `SessionRevocationValidator` still accepts the request). A different browser or IP always makes a distinct session, so the audit trail and the future new-session-from-new-IP alert stay honest. Persistent sessions supersede via their own rotation middleware instead. The dedup targets `!IsPersistent` rows only, so a device that logs in ephemerally and then again with "remember me" legitimately shows two rows — different session kinds, not an oversight. The revoke (`ExecuteUpdateAsync`) and the new-row insert are separate statements with no enclosing transaction; a rare failure between them leaves the old row revoked and no new row, self-recovered by the user's next login attempt.
- **Retention sweep** — `SweepSessions` (`ProjectCeres/Tools/`, `--sweep-sessions` CLI, cron-invoked daily) flat-DELETEs rows past the 90-day horizon: `(RevokedAt IS NOT NULL AND RevokedAt < now − 90d) OR (RevokedAt IS NULL AND LastUsedAt < now − 90d)`, regardless of `IsPersistent` — an abandoned "remember me" row is never revoked (rotation only fires on a return visit), so the `LastUsedAt` clause must apply to persistent rows too or it would never be swept. A **flat cross-tenant** DELETE through `AdminDbContext` (ceres_admin, BYPASSRLS) — `ExecuteDeleteAsync`, not a per-user `IUserJobRunner` fan-out; `AppDbContext`/`ceres_app` would RLS-scope it to one user and delete nothing. Carries `[RequiresAdminContext]`; its `IgnoreQueryFilters()` is allow-listed in `ArchitectureTests`. Deleting the row deletes its `UserAgent`, satisfying both `security-model.md` § Retention lines (revoked sessions + UA strings, 90 days) in one sweep.
...
| PrecedingTicketId | uuid          | FK, NULL      | → SupportTicket. The closed ticket this one continues; null for a standalone ticket. **Part of the composite FK `(PrecedingTicketId, UserId)` → `(Id, UserId)`** (Stage 12.5 A1), so a follow-up cannot reference another user's ticket. `OnDelete: Restrict` |
| CreatedAt         | datetime      | NOT NULL      |                                                                |
...
**Cross-user reference — now structural (`PrecedingTicketId`, Stage 12.5 A1):** PostgreSQL's referential-integrity trigger is not subject to RLS, so a *single-column* FK would let the database accept a `PrecedingTicketId` pointing at another user's ticket (an existence oracle over an unguessable GUID — no content crosses, since reads pass the EF filter + the policy `USING`). **The FK is now composite `(PrecedingTicketId, UserId)` → `(Id, UserId)`**, so that reference is unrepresentable, not merely service-refused — matching the `SupportMessage → SupportTicket` and attachment hops. `PrecedingTicketId` is nullable, so under MATCH SIMPLE a standalone ticket (null predecessor) carries no FK obligation. The service still enforces the *Closed*-and-same-owner rule (`SupportTicketService.CreateAsync`); the schema now guarantees the same-owner half. `Restrict` (not cascade): deleting an earlier ticket must never take its follow-ups. Pinned by `ParityTests.Preceding_ticket_fk_is_scoped_to_the_owner_in_the_database`. (Before A1 this FK was deliberately single-column + service-only, because unlike the cascading hops the exposure was a read-only existence oracle; A1 closed it anyway on the user's direction.)
