...
test('10 wrong passwords → lockout email → unlock link → re-login', async ({ page, request, baseURL }) => {
  ...
  // Exactly one unlock email, with the root-relative link (Stage 11 Task 7).
  const mail = await waitForEmail(user.email, (e) => e.bodyText.includes('/account/unlock#token='))
  ...
  await expect(page).toHaveURL(/\/login\?unlocked=1/, { timeout: 10_000 })
